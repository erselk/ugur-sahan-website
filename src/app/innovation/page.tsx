    ...
    async function fetchWritings() {
      try {
        const { data, error } = await supabase
          .from('writings')
          .select('*')
          .eq('category', 'İnovasyon ve Girişimcilik')
    ...
    return (
      <div className="flex items-center justify-center min-h-[60vh]">
        <p className="text-muted-foreground text-center">
          {language === 'tr' 
            ? 'Henüz inovasyon veya girişimcilik yazısı bulunmamaktadır.'
            : 'No innovation or entrepreneurship content found yet.'
          }
        </p>
      </div>
    );
    ...
    <div className="container max-w-[1920px] py-12 px-6 md:px-8 lg:px-12 space-y-12">
      ...
      >
        <h1 className="text-4xl font-bold tracking-tight">
          {language === 'tr' ? 'İnovasyon ve Girişimcilik' : 'Innovation & Entrepreneurship'}
        </h1>
        <p className="text-muted-foreground text-lg">
          {language === 'tr' 
            ? 'Uğur Şahan\'ın inovasyon ve girişimcilik dünyasından yazılar...'
            : 'Writings from Uğur Şahan\'s world of innovation and entrepreneurship...'
          }
        </p>
      </motion.div>
